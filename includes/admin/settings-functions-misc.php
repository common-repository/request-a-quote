	...
	function emd_misc_settings_tab($tabs,$app){
		...
		if(function_exists('emd_show_login_register_options') 
			...
			$show_tab = 1;
                ...
                if($show_tab == 1){
			$tabs['misc'] = __('Misc', 'request-a-quote');
...
if (!function_exists('emd_misc_tab')) {
	...
	<div class='tab-content' id='tab-misc' <?php if ( 'misc' != $active_tab ) { echo 'style="display:none;"'; } ?>>
		<?php	echo '<form method="post" action="options.php">';
			...
			echo '<input type="hidden" name="' . esc_attr($app) . '_misc_settings[app]" id="' . esc_attr($app) . '_misc_settings_app" value="' . esc_attr($app) . '">';
			...
			$shc_list = get_option($app . '_shc_list');
				...
				echo esc_html__('Google Maps API Key','request-a-quote');
				echo '</label></th><td>';
				$google_mapkey = '';
				if(!empty($misc_settings['google_mapkey'])){
					$google_mapkey = $misc_settings['google_mapkey'];
				}
				echo "<input id='" . esc_attr($app) . "_misc_settings_google_mapkey' name='" . esc_attr($app) . "_misc_settings[google_mapkey]' type='text' value='" . esc_attr($google_mapkey) . "'";
				echo "></input><p class='description'>" . esc_html__('Google requires api key to access google map service. By default we provide a complimentary key. Get your own google maps api key to avoid service interruptions','request-a-quote') . ": <a href='https://developers.google.com/maps/documentation/javascript/get-api-key' target='_blank'>Documentation</a></p></td></tr>";
                        }
			if(!empty($shc_list) && isset($shc_list['has_bs']) && $shc_list['has_bs'] == 1){
				echo "<tr><th scope='row'><label for='misc_settings_disable_bs_css'>";
				echo esc_html__('Disable Bootstrap CSS','request-a-quote');
				echo '</label></th><td>';
				$disable_bs_css =0;
				...
				echo "></input><p class='description'>" . esc_html__('Disables loading of Bootstrap stylesheet in plugin related pages when checked. You may need to disable it if your theme already uses Bootstrap based stylesheet.','request-a-quote') . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='misc_settings_disable_bs_js'>";
				echo esc_html__('Disable Bootstrap JS','request-a-quote');
				echo '</label></th><td>';
				$disable_bs_js =0;
				...
				echo "></input><p class='description'>" . esc_html__('Disables loading of Bootstrap JavaScript in plugin related pages when checked. You may need to disable it if your theme already uses Bootstrap based JavaScript.','request-a-quote') . "</p></td></tr>";
			}
			if(!empty($shc_list) && isset($shc_list['has_autocomplete']) && $shc_list['has_autocomplete'] == 1){
				...
				echo esc_html__('Autocomplete Minimum Input Length','request-a-quote');
				echo '</label></th><td>';
				$min_input = 3;
				if(!empty($misc_settings['autocomp_min_input'])){
					$min_input = $misc_settings['autocomp_min_input'];
				}
				echo "<input id='" . esc_attr($app) . "_misc_settings_autocomp_min_input' name='" . esc_attr($app) . "_misc_settings[autocomp_min_input]' type='text' value='" . esc_attr($min_input) . "'";
				echo "></input></td></tr>";
				echo "<tr><th scope='row'><label for='misc_settings_autocomp_num_result'>";
				echo esc_html__('Autocomplete Number of Results Returned','request-a-quote');
				echo '</label></th><td>';
				$num_result = 5;
				if(!empty($misc_settings['autocomp_num_result'])){
					$num_result = $misc_settings['autocomp_num_result'];
				}
				echo "<input id='" . esc_attr($app) . "_misc_settings_autocomp_num_result' name='" . esc_attr($app) . "_misc_settings[autocomp_num_result]' type='text' value='" . esc_attr($num_result) . "'";
				echo "></input></td></tr>";

			}
			if(function_exists('emd_show_login_register_options') && emd_show_login_register_options($app)){
				echo '<tr><th scope="row"><label for="misc_no_access_msg">' . esc_html__('No Access Message','request-a-quote') . '</label></th>';
				$no_access_msg = (isset($misc_settings['no_access_msg'])) ? $misc_settings['no_access_msg'] : __('You do not have sufficient permissions to access this page.','request-a-quote');
				echo '<td colspan=5><textarea class="large-text code" cols=50 rows=5 id="' . esc_attr($app) . '_misc_settings_no_access_msg" name="' . esc_attr($app) . '_misc_settings[no_access_msg]">' . esc_textarea($no_access_msg) . '</textarea>';
				echo '</td></tr>';
				echo '<tr><th scope="row"><label for="misc_login_reg">' . esc_html__('Show Register / Login Form','request-a-quote') . '</label></th>';
				echo '<td colspan=5><select id="' . esc_attr($app) . '_misc_settings_login_reg" name="' . esc_attr($app) . '_misc_settings[login_reg]">';
				$login_reg_options = Array('none' => __('Only Message','request-a-quote'),
							'both' => __('Registration and Login Forms','request-a-quote'),
							'login' => __('Only Login Form','request-a-quote'));
				...
				echo '</select>';
				echo '<p class="description">' . esc_html__('Display the registration and login forms on the pages non-logged-in users don\'t have access to.','request-a-quote') . '</p></td></tr>';
			}
			echo '</tbody></table>';
			echo '</ul></div>';
			submit_button(); 
