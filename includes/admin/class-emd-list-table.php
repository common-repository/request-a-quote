...
class Emd_List_Table extends WP_List_Table {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$columns = array(
					...
					'name' => esc_html__( 'Name', 'request-a-quote' ),
					'label' => esc_html__( 'Label', 'request-a-quote' ),
					'entity' => esc_html__( 'Entity', 'request-a-quote' ),
					'ctype' => esc_html__( 'Type', 'request-a-quote' ),
					'dtype' => esc_html__( 'Display Type', 'request-a-quote' ),
					'created'   => esc_html__( 'Updated', 'request-a-quote' ),
				);
			}
			else {
				$columns = array(
					'cb'        => '<input type="checkbox" />',
					'name' => esc_html__( 'Name', 'request-a-quote' ),
					'type' => esc_html__( 'Type', 'request-a-quote' ),
					'shortcode' => esc_html__( 'Shortcode', 'request-a-quote' ),
					'created'   => esc_html__( 'Created', 'request-a-quote' ),
				);
				if($this->type == 'form'){
					$columns['modified'] = esc_html__( 'Modified', 'request-a-quote' );
				}
			}
		}
		else {
	...
	public function column_default( $rec, $column_name ) {
		switch ( $column_name ) {
			...
			case 'ctype':
				if($rec['ctype'] == 'attr'){
					$value = __('Attribute','request-a-quote');
				}
				elseif($rec['ctype'] == 'tax'){
					$value = __('Taxonomy','request-a-quote');
				}
				break;
			case 'dtype':
				$value = $rec['dtype'];
			...
			case 'shortcode':
				$value = '<p id="shortcode_' . $rec['id'] . '">' . stripslashes($rec['shortcode']) . '</p>
					<a class="emd-copy-clipb button button-primary" style="padding:1px 6px;" data-clipboard-target="#shortcode_' . $rec['id'] . '">' . __('Copy','request-a-quote') . '</a>';
				break;
			case 'created':
	...
	public function get_bulk_actions() {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$actions = array(
					'delete' => esc_html__( 'Delete Field', 'request-a-quote' ),
					'delete_data' => esc_html__( 'Delete Field & Data', 'request-a-quote' ),
				);
			}
			elseif($this->type == 'form'){
				$actions = array(
					'delete' => esc_html__( 'Delete', 'request-a-quote' ),
					'export' => esc_html__( 'Export', 'request-a-quote' ),
				);
			}
			else {
				$actions = array(
	...
	public function process_bulk_actions() {
		if($this->type  != 'cfield'){
                        $ids    = isset( $_GET['rec_id'] ) ? array_map( 'absint', $_GET['rec_id'] ) : array();
                }
                else {
                        $ids    = isset( $_GET['rec_id'] ) ? array_map( 'sanitize_text_field', $_GET['rec_id'] ) : array();
                }
		...
		if ( 'delete' === $this->current_action() ) {
			if (wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'bulk-' . $this->type . 's')){
				if($this->type  == 'shortcode'){
				...
				<div class="notice updated">
					<p>
						<?php
						if ( count( $ids ) === 1 ) {
							esc_html_e( 'Record was successfully deleted.', 'request-a-quote' );
						} else {
							esc_html_e( 'Records were successfully deleted.', 'request-a-quote' );
						}
						?>
					</p>
				</div>
				...
				<div class="notice updated">
					<p>
						<?php esc_html_e( 'Security check failed. Please try again.', 'request-a-quote' ); ?>
					</p>
				</div>
				<?php
			}
			...
			if(!empty($cfields)){
				foreach($cfields as $kfield => $cfield){
					...
					if(!empty($_GET['orderby'])){
						...
						$sort_data[$kfield] = sanitize_text_field($cfield[$_GET['orderby']]);
					}
				}
				if(!empty($_GET['order']) && $_GET['order'] == 'desc'){
					arsort($sort_data);	
	...
	/**
		...
		if($this->type == 'form'){
			$name = sprintf(
				...
				esc_html__('Edit Form', 'request-a-quote'),
				$name
			);
			...
			$row_actions['edit_settings'] = sprintf(
				...
				esc_html__('Edit Settings', 'request-a-quote'),
				esc_html__('Edit Settings', 'request-a-quote')
			);
			$row_actions['edit'] = sprintf(
				...
				esc_html__('Edit Layout', 'request-a-quote'),
				esc_html__('Edit Layout', 'request-a-quote')
			);
		...
		if($this->type == 'cfield'){
			$name = sprintf(
				...
				esc_html__('Edit', 'request-a-quote'),
				$name
			);
		}
		return $name;
