...
function emd_show_login_register_forms($app,$fcontent,$show){
	if($show != 'none'){
		...
		if(!empty($_GET['emd_error'])){
			switch($_GET['emd_error']){
				case 'invalid_login':
					$error = __('Invalid username, email or password.','request-a-quote');
					break;
			}
		}
		else {
	...
	$log_reg_vars['user_email_msg'] = __('This email has been already registered.', 'request-a-quote');
	$log_reg_vars['verify_msg'] = __('Invalid username','request-a-quote');
	$log_reg_vars['verify_pass'] = __('Please enter at least 5 characters.','request-a-quote');
	$log_reg_vars['verify_pass2'] = __('Please enter same password.','request-a-quote');
	$log_reg_vars['verify_email'] = __('Email address already taken.','request-a-quote');
	...
	if($show != 'none'){
		...
		if (!empty($error)) {
			...
			echo "<div class='emd-alert-error emd-alert'>" . esc_html($error) . "</div>";
			echo "</div>";
		}
		elseif(!empty($fcontent)) {
			...
			if(!empty($noaccess_msg)){
				echo "<div class='emd-alert-container'>";
				echo "<div class='emd-alert-error emd-alert'>" . esc_html($noaccess_msg) . "</div>";
				echo "</div>";
			}
		}
		...
		if(!empty($misc_settings['no_access_msg'])){
			...
			_e('You do not have sufficient permissions to access this page.', 'request-a-quote');
		}
		echo '</div></div>';
	}
}
...
function emd_login_register_verify_email(){
		...
		if(email_exists(sanitize_email($_POST['email']))) {
                        wp_send_json_error(array('msg' => __('Email aready exists','request-a-quote')));
                }
                else {
