...
function emd_login_settings_tab($tabs,$app){
	...
	if($has_login_shc == 1){
		$tabs['login'] = __('Login', 'request-a-quote');
...
?>
	...
	<?php	echo '<form method="post" action="options.php">';
		...
		echo "<h4>" . esc_html__('Login/Register:','request-a-quote') . ' ' . esc_html__('Use the options below to customize Login page.','request-a-quote');
		echo '</h4>';
		...
		echo esc_html__('Login Page','request-a-quote');
		echo '</label></th><td>';
		...
		echo '</select>';
		echo "<p class='description'>" . sprintf(esc_html__('Use [emd_login app="%s"] shortcode in your page. Attributes: ent="emd_customer,emd_vendor" show_reg=1 , reg_label="",reg_link="",show_lost=1  ','request-a-quote'),esc_html($app)) . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='login_settings_pass_reset_subj'>";
		echo esc_html__('Password Reset Message Subject','request-a-quote');
		echo '</label></th><td>';
		...
		echo esc_html__('Password Reset Message','request-a-quote');
		echo '</label></th><td>';
		ob_start();
		...
		echo "<tr><th scope='row'><label for='login_settings_pass_reset_msg'>";
		echo '</label></th><td>';
		echo esc_html__('Use these template tags to customize your email: {username}, {password_reset_link}, {sitename}','request-a-quote'); 
		echo '</td></tr>';
		...
		if(!empty($cust_roles)){
			...
			echo esc_html__('User Role Redirects','request-a-quote');
			echo '</h3>';
			echo "<span class='description' style='padding-left:10px;color:#777;'>- " . esc_html__('Set a redirect url which users belonging to a role will be redirected to after a successful login or logout.','request-a-quote') . "</span></th></tr>";
			$ent_list = get_option($app . '_ent_list');
			foreach($cust_roles as $krole => $vrole){
				...
				echo sprintf(esc_html__('%s Login','request-a-quote'),esc_html($vrole));
				echo '</label></th><td>';
				...
				if(!empty($ent_roles[$krole])){
					echo '<p class="description">' . esc_html__('If left empty, user will be directed to the corresponding user entity page','request-a-quote') . '</p>';
				}
				else {
					echo '<p class="description">' . esc_html__('If left empty, user will be directed to the site\'s homepage','request-a-quote') . '</p>';
				}
				echo '</td></tr>';
				echo "<tr><th scope='row'><label for='login_settings_redirect_logout'>";
				echo sprintf(esc_html__('%s Logout','request-a-quote'),esc_html($vrole));
				echo '</label></th><td>';
				...
				echo '>';
				echo '<p class="description">' . esc_html__('If left empty user, will be directed to the login page','request-a-quote') . '</p>';
				echo '</td></tr>';
			}
		}
		echo '</tbody></table>';
