...
<?php if(!have_posts()){
	echo "<div class='emd-arc-tax-no-records'><div class='emd-arc-tax-no-records-txt'>" . esc_html__('No records have been found matching the term.','request-a-quote') . "</div></div>";
}
?>
</div>
...
?>
	...
	<?php	if ( $wp_query->max_num_pages > 1 ) { ?>
		...
		<?php $pages = paginate_links( array(
			...
			'type' => 'array',
			'prev_text' => wp_kses( __( '<i class="fa fa-angle-left"></i> Previous', 'request-a-quote' ), array( 'i' => array( 
			'class' => array() ) ) ),
			'next_text' => wp_kses( __( 'Next <i class="fa fa-angle-right"></i>', 'request-a-quote' ), array( 'i' => array( 
			'class' => array() ) ) )
		) );
		if(is_array($pages)){
