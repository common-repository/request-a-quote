	...
	class EMD_MB_Image_Advanced_Field extends EMD_MB_Image_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'emd-mb-image-advanced', 'emdmbImageAdvanced', array(
				'frameTitle' => __( 'Select Images', 'request-a-quote' ),
			) );
		}
		...
		{
			$i18n_title = apply_filters( 'emd_mb_image_advanced_select_string', _x( 'Select or Upload Images', 'image upload', 'request-a-quote' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'emd_mb_image_delete_string', _x( 'Delete', 'image upload', 'request-a-quote' ) );
			$i18n_edit   = apply_filters( 'emd_mb_image_edit_string', _x( 'Edit', 'image upload', 'request-a-quote' ) );
			?>
            <script id="tmpl-emd-mb-image-advanced" type="text/html">
				...
				<li id="item_{{{ attachment.id }}}">
					...
					<div class="emd-mb-image-bar">
						<a title="<?php echo esc_attr($i18n_edit); ?>" class="emd-mb-edit-file" href="{{{ attachment.editLink }}}" target="_blank"><?php echo esc_html($i18n_edit); ?></a> |
						<a title="<?php echo esc_attr($i18n_delete); ?>" class="emd-mb-delete-file" href="#" data-attachment_id="{{{ attachment.id }}}">&times;</a>
					</div>
				</li>
				<# } ); #>
			</script>
